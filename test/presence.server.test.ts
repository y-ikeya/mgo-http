...
describe('名簿', () => {
  test('全員の状態を運ぶ', async () => {
    ...
    const late = await new Client(server, 'late', spot(5, 0)).ready()
    late.live()
    await Bun.sleep(500)
...
describe('繋ぎ直し', () => {
  test('resume は名簿のあとに来る', async () => {
    ...
    const back = await new Client(server, 'bob', spot(3, 8)).ready()
    back.live()
    await Bun.sleep(800)
  ...
  test('何度繋ぎ直しても続きから', async () => {
    ...
    for (let round = 0; round < 3; round++) {
      bob.close()
      await Bun.sleep(700)
      bob = await new Client(server, 'bob', spot(0, 6)).ready()
      bob.live()
