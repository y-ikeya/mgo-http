...
#     決め打ちすると毎回ここを直すことになる
#
# **ブルパップは自動判定が外れる。** 銃口側が細く床尾側が太いという前提で
# 見ているが、P90 は機関部と弾倉が後ろ半分を占めるので**逆に出る**。
# 前後が入れ替わったら 4 つ目の引数で明示する (P90 は max)。
#
#   $BLENDER ... -- ~/Downloads/p90_final/scene.gltf public/models/smg.glb 0.5 max
#
# 出来上がりは必ず**真横から撮って確かめる**。既存の rifle.glb を同じ画で
# 撮って並べれば、銃口がどちら向きかは一目で分かる。
#
