...
const PISTOL: WeaponSpec = {
  ...
  /*
   * 減衰が早い。15m を過ぎると落ち始め、45m で半分になる。
   *
   * **頭の即死の射程はここで決まる。** HEAD が 100 (体力ぴったり) なので、
   * 減衰が始まる距離を越えた瞬間に即死ではなくなる。25m にしていた頃は
   * **突撃銃と同じ射程で即死**していて、拳銃を持つ意味が「軽い」だけになっていた。
   *
   * 15m は屋内から路地の向かいくらい。それより先は当てても 1 発では倒れない。
   */
  fullRange: 15,
  minRange: 45,
  minScale: 0.5,
