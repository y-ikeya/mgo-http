 ...
 * sneak クリップ本来の速度 (m/s)。
 *
 * その場歩きで書き出されているので移動量から測れず、歩幅と周期から出す
 * (tools/measure/stride.js)。同じ方法で crouch_f を測ると実測との誤差は 5% だった
 * (歩行は両足が離れる瞬間が無いので歩幅が実移動とほぼ一致する)。
 *
 * 腕を畳んだ型 (SneakingForward.fbx の 2 本目) に替えた: 歩幅 0.79m、周期 1.70s。
 * 前の型は 0.81m / 1.30s で 1.3 だった。
 */
const SNEAK_CLIP_SPEED = 0.93
 ...
 * 足の同期より見え方を取る。
 *
 * 腕を畳んだ型 (周期 1.67s) に替えたら、0.65 では足が空転して見えた
 * (箱の速さ 3.15 m/s ÷ 型の 0.93 m/s × 0.65 で、1 周が 0.76 秒)。0.5 で 1 秒。
 */
const SNEAK_RATE = 0.5
/**
...
/**
 * ダンボールを被って移動する間の追加の前傾 (前へ倒す角度)。
 *
 * 腕を畳んだ sneak (SneakingForward.fbx の 2 本目) は頭頂が 1.27m (肉込み) で、
 * 箱の天井 (1.0m + 浮き 0.3m) にぎりぎり。少し丸めて余裕を作る。
 * 前の型は頭が 1.41m あって 34° 要ったが、それは下の符号が逆で**頭を上げて
 * いた**ので、実際には効いていなかった。
 *
 * **軸の正は後ろ。** lean はキャラの右手側の軸まわりに足していて、正の回転は
 * 頭を後ろ (のけぞり) へ倒す。試写を本番と同じ入れ子 (根 → 模型 180°) に
 * して描いたら、+34° で頭が上がり、-34° で下がった。tools/measure/tilt.ts と
 * 以前の試写は模型を直に置いていて軸が左右逆だったので、そこで測った
 * 「前傾」は本番では後ろだった。ここでは引いて前へ倒す。
 *
 * 掛けるのは移動中だけ。座り姿勢は頭が 0.59m しかなく、そもそも箱に収まっている。
 */
const BOX_LEAN = THREE.MathUtils.degToRad(15)
