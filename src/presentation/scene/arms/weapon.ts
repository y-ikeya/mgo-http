...
const SNIPER: WeaponConfig = {
  ...
  /*
   * しゃがみ。**型 (chrouchFire) の中の置き場所から出した値。**
   *
   * 目で詰めた値は 12.8cm / 11.6° ずれていた。動きを作る側は銃を右手の骨へ
   * 直付けして合わせているので、そちらが正しい (tools/fit_gun_to_clip.js)。
   */
  crouchGrip: new THREE.Vector3(-0.0175, 0.2801, 0.1392),
  crouchRotation: new THREE.Euler(degrees(-8.2), degrees(-7.8), degrees(176.9)),
