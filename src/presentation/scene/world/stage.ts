 ...
 * 高所へ上がる階段を段の配列にする。
 *
 * 一段を「歩いて越えられる段差」(domain/player/moving.ts の STEP_UP = 0.25m) ちょうどにしてある。
...
/**
 * 影を落とす範囲の半径 (m)。**光から見た平面での半径。**
 *
 * --- 45 では足りていなかった ---
 * 「壁が ±40m だから ±45 で全域が入る」と書いてあったが、**それは世界の座標
 * での話**。影は光から見た平面へ焼くので、斜めから見たぶん広がる。実測すると
 * 横に 56.6m、縦に 64.7m 要る (壁が 32m あるので、その影が長く伸びる)。
 *
 * 訓練場で「端の影が途中で切れる」として出た。モールも同じだったが、
 * 建物で埋まっているぶん目立たなかった。
 *
 * 実測は tools ではなく、書き出した stage_*.json の箱を光の座標へ移して測る。
 * ステージを描き替えたら測り直す。
 */
const SHADOW_RADIUS = 65
...
export function buildLights(scene: THREE.Scene): THREE.DirectionalLight {
  ...
  //
  // 影の範囲が全域を覆っているので、動かす理由が無い。ステージがこれより
  // 広くなったら、追従ではなく影マップを分ける (カスケード) 方を考える。
  //
  // --- 光源を引いてある ---
  // (18, 30, 12) に置いていた。**壁が 32m あるので光より高く**、光の後ろへ
  // 回った部分が near で切られて影を落としていなかった。同じ向きのまま
  // 4 倍の距離へ引くと、全部が光の前に来る (実測で奥行き 88.8〜181.1)。
  //
  // 平行光なので、引いても光の向きも明るさも変わらない。
   ...
   * 影マップの画素数。
   *
   * 範囲を 45 から 65 へ広げたぶん、2048 のままだと 1m あたり 22.8 画素から
   * 15.8 画素へ落ちる。4096 にすれば 31.5 画素で、**広げた上で前より細かい**。
