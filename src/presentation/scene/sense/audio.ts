...
const SOUNDS = {
  ...
  /*
   * 囮の人形が割れた音。**普通の位置音として鳴らす。**
   *
   * 特別扱いしないのが肝で、近ければ聞こえ遠ければ聞こえない。それだけで
   * 過不足のない情報が両側へ渡る:
   *
   ...
   *
   * **届く距離は銃声と同じ 34m。** 揃えると規則が 1 つで済む —
   * 「その銃声が聞こえる所に居たなら、破裂音も聞こえる」。撃った側から見て
   * 「聞こえたなら晒された」の線が銃声の届く範囲と一致するので、
   * 覚えることが増えない。
   */
  balloon: { file: "balloon1.mp3", reference: 6, max: 34 },
  /*
   * 人形が膨らむ音。**破裂音より遥かに近くまでしか届かない。**
   *
   * 膨らんでいる 2 秒は罠が一番弱い所で、見られたら死ぬ。音まで遠くへ
   * 届くと「置いた」こと自体が漏れて、置く場所を選ぶ意味が薄くなる。
   * **すぐ近くを通った人だけが気づく**くらいで足りる。
   */
