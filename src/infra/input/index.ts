...
export class Input {
  ...
  /**
   * 押した瞬間。**長押しの割り当てがあっても、押した時点で立つ。**
   *
   * `tapped` は離すまで待ち、長く押すと**一度も立たない**。押し始めに
   * 効かせたい操作 (梯子から手を離す) はこちらを使う — 掴んだ手を離すのに
   * 「短く押す」を要求すると、慌てて押し込んだときに落ちられない。
   *
   * 消費するので 1 回の押下につき 1 回だけ true。
   */
  pushed(action: Action): boolean {
    return this.rawPressed(action)
  }
