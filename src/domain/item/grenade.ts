...
/**
 * 手を離れる高さ (m)。足元からの差。**構えで変わる。**
 *
 * --- なぜここに置くか ---
 * 読む所が 3 つある。サーバー (実際に飛ばす)、画面の予測線、そして投げの絵。
 * **ずれると、見えている落下点と落ちる場所が食い違う。** 手榴弾は「そこへ
 * 落とす」判断そのものが手なので、ずれた時点で武器として壊れる — 速さを
 * こちらへ寄せたのと同じ理由 (throwSpeedOf の注)。
 *
 * --- 数字 ---
 * **合成した後の手の位置**を測った値。放す瞬間 (下の割合) まで動かして、
 * 実際に描かれる骨から読んでいる — 素材のクリップ単体ではない。上半身は
 * 腰を載せ替えて混ぜている (alignSpineToUpperClip) ので、素材だけ見ると
 * 伏せのほうが 0.08m 高く出る。
 *
 *     立ち   1.73m (腰 0.99m)
 *     伏せ   0.80m (腰 0.21m)
 *
 * 伏せだけ別に持つ。しゃがみは立ちの型を屈んだ腰に載せているだけで、
 * **専用の型が無い**ので分けても実際の手の高さと合わない。型が来たら分ける。
 */
export const RELEASE_HEIGHT: Record<Stance, number> = {
  stand: 1.7,
  crouch: 1.7,
  box: 1.7,
  prone: 0.8,
  // 倒れている間は投げられない。表を埋めるためだけの値
  down: 1.7,
}
