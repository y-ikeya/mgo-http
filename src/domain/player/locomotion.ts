...
export type Locomotion =
  ...
  | 'lean_crouch_left'
  | 'lean_crouch_right'
  /** 窓枠や低い塀を跳び越える。転がりの代わりに出る全身の型 */
  | 'vault'
  /** 窓枠の先が同じ高さの床や箱なら、跳び越えずに一段上へ乗る */
  | 'vault_up'
  /** 縁から落ちて手を掛ける。型の最後のコマのままぶら下がる */
  | 'hang_drop'
  /** 縁に手を掛けて待つ。登る型の頭の 1 枚で止める (壁向き、手が前) */
  | 'hang'
  /** ぶら下がりから縁の上へ登り、しゃがみで復帰する */
  | 'hang_climb'
  // 敬礼。全身の型なので上下を分けない
  | 'salute'
  /**
   ...
   * 仰向けからうつ伏せへ、横へ半回転する。
   *
   * 吹き飛ばされた所から這い出す繋ぎ。転ぶ型 (sweep) は**仰向けで終わる**
   * ので、そのまま這う型へ渡すと 1 フレームで裏返る。
   */
  | 'prone_roll_down'
  /**
